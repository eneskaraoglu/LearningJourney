...
## Learning Outcomes
- Explain the core implementation decisions in Data Consistency Distributed with concrete examples.
...
## Deep Dive
### Data Model Clarity
Design entities with explicit ownership and lifecycle boundaries. Avoid accidental bidirectional complexity.

### Transaction Scope
Keep transactions short and purposeful. Long transactions increase lock contention and failure surface area.

### Query Safety
Use parameterized queries and index-aware access patterns to avoid latency cliffs.

### Consistency Decisions
State where strong consistency is required and where eventual consistency is acceptable.
...
public OrderResult placeOrder(CreateOrderCommand cmd) {
  ...
  Order order = new Order(user, cmd.items());
